...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in self.token_indexes:
                token_dict["block_"+str(idx)+"_patchtokens"] = x[0][:, self.n_storage_tokens + 1 :]   # [B,H*W,768]
        ...
        for idx, (x, masks) in enumerate(zip(all_x, masks_list)):
            ...
            token_dict["x_norm_patchtokens"] =  x_norm_patch              # [B,H*W,768]
            token_dict["x_norm_clstoken"] = x_norm_cls_reg[:, 0]
            token_dict["x_storage_tokens"] = x_norm_cls_reg[:, 1:]
