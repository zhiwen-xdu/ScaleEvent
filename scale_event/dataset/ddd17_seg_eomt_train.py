...
class DDD17_SEG_EOMT_TRAIN(torch.utils.data.Dataset):
    ...
    def _get_data(self, seq_id, frame_id):
        ...
        target = {
            ...
            "labels": torch.tensor(labels),                # [C]: [0,1,2,3,4,5]
            "seg_masks": seg_masks
        }
    ...
    def __len__(self):
        return sum(self.sequence_lens)*2  # sum(self.sequence_lens)*5
        # return int(sum(self.sequence_lens) * 0.1)
    ...
    def __getitem__(self, index):
        ...
        # ratio_frame_id = (frame_id // 100) * 100   # 20%(5), 10%(10), 5%(20), 1%(100)
        # print(frame_id,ratio_frame_id)
        event,target = self._get_data(seq_id, frame_id)
        return event,target  # [3,H,W],[H,W]


if __name__ == "__main__":
    dataset = DDD17_SEG_EOMT_TRAIN(split="train")
