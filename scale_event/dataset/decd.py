...
class Decd(torch.utils.data.Dataset):
    def __init__(self,W=640,H=480,S=1):
        self.root = env_settings().decd_dir
        self.W = W
        self.H = H
        self.S = S

        file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data_specs', 'decd.txt')
        with open(file_path) as f:
            seq_names = [line.strip() for line in f.readlines()]
        self.sequence_list = [i for i in seq_names]

        len_file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data_specs', 'decd_seqlen.txt')
