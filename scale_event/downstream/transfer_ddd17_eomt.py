...
from scale_event.dataset import DDD17_SEG_EOMT_TRAIN,DDD17_SEG_EOMT_TEST
...
def transfer(experiment_id="SEG01-DDD17",epoch_num=10,batch_size=16):
    ...
    # Build dataloaders
    dataset_train = DDD17_SEG_EOMT_TRAIN(split="train",W=704,H=416)
    dataset_val = DDD17_SEG_EOMT_TEST(split="test",W=704,H=416)
    ...
    dataloader_val = torch.utils.data.DataLoader(dataset=dataset_val, batch_size=1, num_workers=2,pin_memory=True, sampler=sampler_val)

    # Create the teacher and student encoders
    segmentor = EoMT_ViTL(h=26,w=44,num_classes=6)

    # Set trainable params
    train_params_embed = ["patch_embed"]
    # train_params_backbone = ["blocks." + str(i) + ".mlp" for i in [2, 5, 8, 11]]
    ...
    weight = "/home/zhiyu/projects/DINOv3/scale_event/output/DINO20/checkpoints/ViTL16_Event_Encoder_EP05.pth"
    ...
    # Set loss functions
    loss_fun = SegmentLoss_MOET(num_classes=6, ignore_index=255)
    ess_metric_fun = MetricsSemseg_ESS(num_classes=6, ignore_label=255,class_names=['flat', 'background', 'object', 'vegetation', 'human', 'vehicle'])
    eomt_metric_fun = MetricsSemseg_EoMT(num_classes=6, ignore_label=255)
    ...
    for epoch in range(epoch_num):
        ...
        for i, (events, targets) in enumerate(dataloader_train):
            ...
            if iteration % 10 == 1:
                log_str = "[Train: %d], [Iteration: %d], Segment Loss: %.4f, Mask Loss: %.4f, Dice Loss: %.4f, CE Loss: %.4f" % (
                epoch + 1, iteration, loss.item(), loss_masks.item(), loss_dice.item(), loss_cross_entropy.item())
        ...
        eomt_log_str = "[Val: %d], [Iteration: %d], EoMT-Metrics:" % (epoch + 1, iteration) + "  miou:" + str(
            iou_all * 100) + "  iou_per_class:" + str([i * 100 for i in iou_per_class])
        with open(log_file, 'a') as f:
...
if __name__ == '__main__':
    ...
    transfer(experiment_id="SEG54-DDD17",epoch_num=50, batch_size=12)  # S-60/35, B-25, L-12