...
from scale_event.dataset import DSEC_SEG_TRAIN,DSEC_SEG_TEST
from scale_event.models import Segmentor_01
...
def transfer(experiment_id="SEG01-DSEC",epoch_num=10,batch_size=16):
    ...
    # Build dataloaders
    dataset_train = DSEC_SEG_TRAIN(split="train",W=960,H=672)
    dataset_val = DSEC_SEG_TEST(split="test",W=960,H=672)
    ...
    segmentor = Segmentor_01(encoder='dinov3_vitl16',nclass=11)

    # Set trainable params
    train_params_embed = ["patch_embed"]
    train_params_backbone = ["blocks." + str(i) + ".mlp" for i in [4, 11, 17, 23]]  # [4, 11, 17, 23],[2, 5, 8, 11]
    # train_params_backbone = ["blocks." + str(i) + "." for i in [2, 5, 8, 11]]
    # train_params_backbone = ["blocks." + str(i) + ".mlp" for i in range(12)]
    train_params_head = ["head"]
    ...
    # Set loss functions
    loss_fun = SegmentLoss_ESS(gamma=2.0, num_classes=11, ignore_index=255, reduction='mean')
    metric_fun_val = MetricsSemseg_ESS(num_classes=11, ignore_label=255, class_names=['background', 'building','fence','person','pole','road','sidewalk','vegetation','car','wall','traffic sign'])
...
if __name__ == '__main__':
    ...
    transfer(experiment_id="SEG10-DSEC",epoch_num=50,batch_size=12)  # 60, 26, 15
